...
from backend.apps.library.models import Writer, Book, Review, Slider
from django.contrib.auth import authenticate, login, logout

from django.contrib import messages
from django.core.paginator import EmptyPage, PageNotAnInteger, Paginator
from backend.apps.library.forms import RegistrationForm, ReviewForm


def index(request):
    newpublished = Book.objects.order_by('-created')[:15]
    slide = Slider.objects.order_by('-created')[:3]
    categirie = Category.objects.all()
    context = {
...
def get_book(request, id):
    ...
    return render(request, "stores/book.html", context)
...
def get_books(request):
    ...
    return render(request, "stores/category.html", {"book": books})
...
def get_book_category(request, id):
    ...
    return render(request, "stores/category.html", {"book": book})
...
def get_writer(request, id):
    ...
    return render(request, "stores/writer.html", context)
